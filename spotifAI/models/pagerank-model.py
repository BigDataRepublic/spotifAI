...
df_full = pd.read_csv("/home/ridvanz/repositories/spotifAI/spotifAI/data/raw/Database_to_calculate_popularity.csv")
df_final = pd.read_csv("/home/ridvanz/repositories/spotifAI/spotifAI/data/raw/Final database.csv")
...
tuner = optuna_lgb.LightGBMTunerCV(
    parameters, dtrain, verbose_eval=0, early_stopping_rounds=10, nfold = 3, return_cvbooster=False, time_budget=6000
    )
...
paired0 = np.concatenate([feature_df.loc[pairs[0]].values, feature_df.loc[pairs[1]].values],axis=1)
paired1 = np.concatenate([feature_df.loc[pairs[1]].values, feature_df.loc[pairs[0]].values],axis=1)
...
X_train, X_test, y_train, y_test = train_test_split(paired, targets, test_size=0.2, random_state=42, stratify=targets)
del paired, targets
#%%
...
for k in range(len(groups)):
    ...
    edge_weights[(groups[k].index.values[None,:]), (groups[k].index.values[:,None])] += rank_diffs
...
# %% [markdown]
# ## New data
groups = [group[1].drop("date",axis=1).astype(np.float32) for group in df.groupby("date")]
...
scores = cross_val_score(forest, train_x, train_y, cv=5, scoring = 'neg_root_mean_squared_error')
